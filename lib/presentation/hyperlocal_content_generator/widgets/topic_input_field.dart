...
class _TopicInputFieldState extends State<TopicInputField> {
  ...
  List<String> _getTopicSuggestions(String subject, int grade) {
    final Map<String, Map<int, List<String>>> curriculumTopics = {
      ...
      'Social Studies': {
        3: ['My Family', 'Community Helpers', 'Our Environment', 'Festivals'],
        4: ['Local Geography', 'Historical Places', 'Government', 'Transport'],
        5: ['Indian States', 'Freedom Fighters', 'Constitution', 'Agriculture'],
        6: ['Ancient India', 'Medieval India', 'Geography of India', 'Civics'],
        7: ['Modern India', 'World Geography', 'Democracy', 'Economics'],
        8: ['Indian History', 'Resources', 'Social Issues', 'Governance'],
      },
      'English': {
        3: [
          'Phonics & Spelling',
      ...
      'Environmental Studies': {
        ...
        4: ['Food', 'Shelter', 'Travel', 'Things We Make'],
        5: [
          'Super Senses',
  ...
  void _onTextChanged(String value) {
    if (value.isNotEmpty) {
      setState(() {
        _suggestions =
            _getTopicSuggestions(widget.selectedSubject, widget.selectedGrade)
                .where(
                  (topic) => topic.toLowerCase().contains(value.toLowerCase()),
                )
                .toList();
